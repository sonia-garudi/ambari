...
App.WizardStep3Controller = Em.Controller.extend(App.ReloadPopupMixin, App.CheckHostMixin, {
    ...
    hosts.forEach(function (_host, index) {
      ...
      switch (_host.get('bootStatus')) {
        case 'DONE':
          ...
          _host.set('os_type', jsonData.items.findProperty('Hosts.host_name', _host.name).Hosts.os_type);
  ...
  getJDKName: function () {
    ...
    this.get('bootHosts').forEach(function (host) {
      if (osTypeJDKHomeField.indexOf(host.os_family) == -1) {
        osTypeJDKHomeField = osTypeJDKHomeField.concat(",RootServiceComponents/properties/java.home.", host.os_family);
      }
    }, this);
    return App.ajax.send({
      name: 'ambari.service',
      sender: this,
      data: {
        fields: '?fields=RootServiceComponents/properties/jdk.name,RootServiceComponents/properties/java.home,RootServiceComponents/properties/jdk_location' + osTypeJDKHomeField
      },
      ...
      data.tasks.forEach( function(task) {
        ...
        if (Em.get(task, "Tasks.structured_out.java_home_check.exit_code") == 1){
          ...
          for (var osFamily in this.javaHomeHostInfo) {
            if (osFamily.contains(warnedHost.os_family)) {
              this.javaHomeHostInfo[osFamily].hosts.push(task.Tasks.host_name);
              this.javaHomeHostInfo[osFamily].host_jdk_context.push(jdkContext);
              break;
            }
          }
        }
  ...
  /**
   * Checks allRepos if repository exists for os_type and sets promptRepoInfo accordingly
   * @method checkRepoForNewOsType
   */
  checkRepoForNewOsType : function () {
    ...
    hosts.forEach(function (_host) {
      ...
      for (var i = 0; i < this.allRepos.length; i++) {
        if (_host.os_family.contains(this.allRepos[i].os_type)) {
          found = true;
          break;
        }
      }
