...
describe('App.WizardStep3Controller', function () {
  ...
  describe('#getSupportedOSList', function () {

    it('should do request for all supported os types for installer', function (){
      var controller = App.WizardStep3Controller.create({
        content: {
          controllerName: 'installerController'
        }
      });
      sinon.stub(App.Stack, 'find').returns([Em.Object.create({isSelected: true, id: 'HDP-2.1'})]);
      controller.getSupportedOSList();
      App.Stack.find.restore();
      var args = testHelpers.findAjaxRequest('name', 'wizard.get_version_definition');
      expect(args).exists;
    });

    it('should do request for all supported os types for addhost', function (){
      var controller = App.WizardStep3Controller.create({
        content: {
          controllerName: 'addHostController'
        }
      ...
      sinon.stub(App.StackVersion, 'find', function() {
        return [
                Em.Object.create({stack: 'HDP', version: '2.3', repositoryVersion: {repositoryVersion: '2.3.0.0-2208'}})
                ];
      });
      controller.getSupportedOSList();
      App.get.restore();
    ...
    it('should do request for all supported os types for addhost(default stack)', function (){
      var controller = App.WizardStep3Controller.create({
        content: {
          controllerName: 'addHostController'
        }
      ...
      var stubForGet = sinon.stub(App, 'get');
      stubForGet.withArgs('currentStackName').returns('HDP');
      stubForGet.withArgs('currentStackVersionNumber').returns('2.1');
      sinon.stub(App.StackVersion, 'find', function() {
        return [
                Em.Object.create({stack: 'HDP', version: '2.1', repositoryVersion: {repositoryVersion: '2.3.0.0-2208'}})
                ];
      });
      controller.getSupportedOSList();
      App.get.restore();
  ...
  describe("#checkRepoURLInstallerSuccessCallback", function () {
    ...
    it('should set validation to OK', function () {
      var os = c.get('newSupportedOsList').findProperty('os_type', params.osType);
